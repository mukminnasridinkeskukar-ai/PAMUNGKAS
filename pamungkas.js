  ...
  cekPendaftaran: `
    query CekPendaftaran($nik: String!) {
      pendaftaran(where: {nik: {_eq: $nik}}) {
        id nama_lengkap_dengan_gelar nik nip unit_kerja jenis_profesi
        email_plataran_sehat judul_kegiatan status created_at
        catatan_admin
      }
    }
  `,
      ...
      switch(action) {
        ...
        case 'getPendaftaran':
          ...
          resolve({ success: true, data: (result.pendaftaran || []).map(p => ({
            ...
            'Lama Bekerja': p.lama_bekerja_di_unit_sekarang, WhatsApp: p.nomor_whatsapp,
...
function renderMateriCards(data){
  ...
  data.forEach(function(r, i) {
    ...
    var link = _getSertField(r, ['Link/File', 'Link', 'link_download', 'link', 'Link_File']);
...
function submitMateri(e){e.preventDefault();var idx=parseInt(document.getElementById('materiEditIndex').value);var data={'Judul Materi':document.getElementById('matJudul').value,Kategori:document.getElementById('matKategori').value,'Link/File':document.getElementById('matLink').value};
  showLoading('Menyimpan...');
  var done=function(res){hideLoading();closeModal('materiFormModal');showToast(res.message,res.success?'success':'error');if(res.success)loadMateri();};
  var fail=function(e){hideLoading();showToast('Error: '+(e.message||e),'error');};
...
/**
 * renderDynamicSidebar() - Render sidebar menu berdasarkan role
...
function renderDynamicSidebar() {
  ...
  var role = getCurrentRole();
  console.log('[ADMIN] Rendering dynamic sidebar for role:', role);
  
  var html = '';
  var visibleMenus = [];
  
  // Default menu untuk semua user (belum login)
  if (!role || !isValidRole(role)) {
    // Menu publik tanpa Panel Admin
    html += '<div class="nav-label">Menu Utama</div>';
    html += '<a class="nav-item" data-page="dashboard" onclick="navigateTo(\'dashboard\')"><i class="fas fa-th-large"></i><span>Dashboard</span></a>';
    html += '<a class="nav-item" data-page="pengumuman" onclick="navigateTo(\'pengumuman\')"><i class="fas fa-bullhorn"></i><span>Pengumuman</span></a>';
    html += '<a class="nav-item" data-page="profil-sdmk" onclick="navigateTo(\'profil-sdmk\')"><i class="fas fa-user-md"></i><span>Profil SDMK Terlatih</span></a>';
    html += '<a class="nav-item" data-page="pendaftaran" onclick="navigateTo(\'pendaftaran\')"><i class="fas fa-clipboard-list"></i><span>Pendaftaran</span></a>';
    html += '<a class="nav-item" data-page="cek-sertifikat" onclick="navigateTo(\'cek-sertifikat\')"><i class="fas fa-certificate"></i><span>Cek Sertifikat</span></a>';
    html += '<a class="nav-item" data-page="cek-materi" onclick="navigateTo(\'cek-materi\')"><i class="fas fa-book-open"></i><span>Cek Materi</span></a>';
    html += '<a class="nav-item" data-page="cek-pendaftaran" onclick="navigateTo(\'cek-pendaftaran\')"><i class="fas fa-search-location"></i><span>Cek Pendaftaran</span></a>';
    ...
    console.log('[ADMIN] Visible menus (public):', ['Dashboard', 'Pengumuman', 'Profil SDMK', 'Pendaftaran', 'Cek Sertifikat', 'Cek Materi', 'Cek Pendaftaran', 'Login']);
    return;
  }
  
  // Get menu config for this role
  ...
  if (menuConfig.mainMenu && menuConfig.mainMenu.length > 0) {
    html += '<div class="nav-label">Menu Utama</div>';
    menuConfig.mainMenu.forEach(function(menu) {
  ...
  if (menuConfig.panelAdmin && menuConfig.panelAdmin.length > 0) {
    html += '<div class="nav-divider"></div>';
    html += '<div class="nav-label">Panel Admin</div>';
  ...
  // === DATA UTAMA / DATA (Superadmin & Admin) ===
  if (menuConfig.dataUtama || menuConfig.data) {
    var dataMenus = menuConfig.dataUtama || menuConfig.data;
    if (dataMenus.length > 0) {
      html += '<div class="nav-divider"></div>';
      html += '<div class="nav-label">' + (menuConfig.dataUtama ? 'DATA UTAMA' : 'DATA') + '</div>';
      dataMenus.forEach(function(menu) {
        // Only show if user has read permission for this module
  ...
  // === INFORMASI (Superadmin only) ===
  if (menuConfig.informasi && menuConfig.informasi.length > 0) {
    html += '<div class="nav-divider"></div>';
    html += '<div class="nav-label">INFORMASI</div>';
    menuConfig.informasi.forEach(function(menu) {
      if (hasPermission(role, menu.module, 'read')) {
        html += '<a class="nav-item" onclick="openAdminCrud(\'' + menu.module + '\')"><i class="fas ' + menu.icon + '"></i><span>' + menu.label + '</span></a>';
        visibleMenus.push(menu.label);
      }
  ...
  if (menuConfig.sistem && menuConfig.sistem.length > 0) {
    html += '<div class="nav-divider"></div>';
    html += '<div class="nav-label">SISTEM</div>';
    menuConfig.sistem.forEach(function(menu) {
      if (hasPermission(role, menu.module, 'read')) {
        html += '<a class="nav-item" onclick="openAdminCrud(\'' + menu.module + '\')"><i class="fas ' + menu.icon + '"></i><span>' + menu.label + '</span></a>';
        visibleMenus.push(menu.label);
      }
  ...
  console.log('[ADMIN] Visible menus for ' + role + ':', visibleMenus);
  console.log('[AUTH] Current user:', currentUser);
  console.log('[AUTH] Current role:', currentRole);
...
/**
 * Open CRUD interface for a specific table
 * Uses RBAC permission system to control access
 * SPECIAL: For 'pendaftaran', shows TAB system (Data Pendaftaran | Input Massal)
 */
function openAdminCrud(type) {
  console.log('[ADMIN] Opening CRUD for:', type);
  
  // Use requirePermission for access control (blocks + shows error)
  if (!requirePermission(type, 'read')) {
    return; // Error message already shown by requirePermission
  }
  
  var tableConfig = ADMIN_TABLES[type];
  if (!tableConfig) {
    console.error('[ADMIN] Table config not found for:', type);
    showToast('Konfigurasi tabel tidak ditemukan: ' + type, 'error');
    return;
  }
  ...
  var titles = {};
  Object.values(ADMIN_TABLES).forEach(function(t) { titles[t.id] = 'Kelola ' + t.label; });
  
  // Check specific permissions for this module
  var canCreateData = canCreate(type);
  var canUpdateData = canUpdate(type);
  var canDeleteData = canDelete(type);
  var canBulkImport = hasPermission(getCurrentRole(), type, 'bulk_import');
  var writable = canCreateData || canUpdateData || canDeleteData;
  ...
  // ============================================================
  // SPECIAL HANDLING: PENDAFTARAN with TAB SYSTEM
  // ============================================================
  if (type === 'pendaftaran') {
    renderPendaftaranTabSystem(c, tableConfig, titles, writable, canBulkImport, readOnly);
    return;
  }
  
  // Build info bar based on permissions (for non-pendaftaran modules)
  var role = getCurrentRole();
  var infoBarHtml = '';
  if (readOnly) {
    infoBarHtml = '<div class="admin-info-bar" style="background:var(--accent-light);border-left:4px solid var(--accent);padding:12px;border-radius:8px;margin-bottom:16px;">' +
      '<i class="fas fa-eye" style="color:var(--accent);margin-right:8px;"></i>' +
      'Anda login sebagai <strong>' + levelLabel() + '</strong> — mode <strong>hanya lihat</strong>. ' +
      'Tombol tambah, edit, dan hapus tidak ditampilkan.</div>';
  } else {
    // Show what operations are allowed
    var allowedOps = [];
    if (canCreateData) allowedOps.push('<span class="badge" style="background:var(--secondary);color:#fff;padding:2px 6px;border-radius:4px;font-size:.65rem;margin:0 2px;"><i class="fas fa-plus"></i> Tambah</span>');
    if (canUpdateData) allowedOps.push('<span class="badge" style="background:#F59E0B;color:#fff;padding:2px 6px;border-radius:4px;font-size:.65rem;margin:0 2px;"><i class="fas fa-edit"></i> Edit</span>');
    if (canDeleteData) allowedOps.push('<span class="badge" style="background:var(--danger);color:#fff;padding:2px 6px;border-radius:4px;font-size:.65rem;margin:0 2px;"><i class="fas fa-trash"></i> Hapus</span>');
    
    if (allowedOps.length > 0) {
      infoBarHtml = '<div class="admin-info-bar" style="background:var(--secondary-light);border-left:4px solid var(--secondary);padding:12px;border-radius:8px;margin-bottom:16px;">' +
        '<i class="fas ' + levelIcon() + '" style="color:var(--secondary);margin-right:8px;"></i>' +
        '<strong>' + levelLabel() + '</strong> — Akses: ' + allowedOps.join(' ') + '</div>';
    }
  }
  
  // Add Import button only if user has create permission
  var importBtn = (writable && ['pengumuman','sdmk','sertifikat','materi','indikator'].includes(type))
  ...
  c.innerHTML = '<div class="page-header">' +
    '<h2><i class="fas ' + tableConfig.icon + '" style="margin-right:8px;color:var(--primary);"></i>' + 
    (titles[type] || type) + '</h2>' +
    '<p>Kelola data <strong>' + tableConfig.table + '</strong> dari database Nhost.</p>' + 
    importBtn + '</div>' + 
    infoBarHtml + 
    '<div id="adminCrudTable"></div>';
  
  console.log('[ADMIN] Loading table:', type, '→ Action:', tableConfig.queryAction);
  
  // Special handling for different table types
  if (type === 'multiusers' || type === 'admin') {
    loadAdminList();
    return;
  }
  ...
  // For other tables, use the existing action mapping
  ...
  showLoading('Memuat data ' + tableConfig.label + '...');
  
  callServer(actionMap[type]).then(function(res) {
    hideLoading();
    
    if (!res || !res.success) {
      document.getElementById('adminCrudTable').innerHTML = 
        '<div class="empty-state"><i class="fas fa-exclamation-triangle" style="font-size:2rem;color:var(--accent);"></i><p>' + 
        escHTML(res ? res.message : 'Gagal memuat data') + '</p></div>';
      return;
    }
    ...
    if (_crud.allData.length > 0) {
      ...
      _crud.headers = tableConfig.fields || [];
    }
    
    // Gunakan displayFields dari config (maks 7 kolom), filter hanya yang ada di data
    var availableFields = _crud.headers;
    if (tableConfig.displayFields && tableConfig.displayFields.length > 0) {
      _crud.visibleCols = tableConfig.displayFields.filter(function(f) {
        return availableFields.indexOf(f) !== -1;
      });
      // Jika setelah filter kosong, fallback ke semua headers
      if (_crud.visibleCols.length === 0) {
        _crud.visibleCols = [...availableFields];
      }
    } else {
      _crud.visibleCols = [...availableFields];
    }
    ...
    console.log('[ADMIN] Data loaded:', _crud.allData.length, 'records from', tableConfig.table);
    
  }).catch(function(e) {
    hideLoading();
    console.error('[ADMIN] Error loading', type, ':', e);
    showToast('Gagal memuat data: ' + (e.message || e), 'error');
    document.getElementById('adminCrudTable').innerHTML = 
      '<div class="empty-state"><i class="fas fa-exclamation-circle" style="font-size:2rem;color:var(--danger);"></i><p>Error: ' + 
...
function showValidationResults(data) {
  ...
  actionsContainer.innerHTML = 
    '<button class="btn btn-secondary" onclick="resetImportState()">' +
...
/**
 * formatFileSize() - Format ukuran file ke readable string
 */
...
function searchCekPendaftaran(){
  ...
  callServer('getPendaftaran').then(function(res){
    ...
    for(var i=0;i<all.length;i++){
      var r=all[i];
      var nik=String(r['NIK']||r['NIK/NIP']||r.NIK_NIP||r['NIK (Nomor Induk Kependudukan)']||'').trim().toLowerCase();
      var nip=String(r['NIP']||r['NIK/NIP']||r.NIK_NIP||r['NIP (Nomor Induk Pegawai)']||'').trim().toLowerCase();
      if(nik===ql||nip===ql||nik.indexOf(ql)!==-1||nip.indexOf(ql)!==-1){
        found.push({data:r,idx:i});
      }
    }
    if(!found.length){
    ...
    _cekRegMultiData=found;
    if(found.length===1){
      _cekRegData=found[0].data;
      _cekRegOrigIdx=found[0].idx;
      renderCekRegResult(ct,_cekRegData,_cekRegOrigIdx);
    }else{
      renderCekRegMultiResult(ct,found,q);
    }
  }).catch(function(e){
...
<!-- Stat Card Popup -->
<div class="stat-popup-overlay" id="statPopupOverlay" onclick="if(event.target===this)closeStatPopup()">
  <div class="stat-popup-box">
    <div class="stat-popup-header">
      <h3 id="statPopupTitle">Detail Data</h3>
      <button class="stat-popup-close" onclick="closeStatPopup()">&times;</button>
    </div>
    <div class="stat-popup-body" id="statPopupBody"></div>
  </div>
</div>

<!-- ==================== BULK IMPORT MODAL ==================== -->
<div class="modal" id="bulkImportModal">
  <div class="modal-overlay" onclick="closeBulkImportModal()"></div>
  <div class="modal-content modal-lg">
    <div class="modal-header">
      <h3><i class="fas fa-file-excel"></i> Import Data Massal</h3>
      <button class="modal-close" onclick="closeBulkImportModal()">&times;</button>
    </div>
    
    <!-- Step 1: Select Module & Upload -->
    <div id="importStep1" class="import-step">
      <h4>Langkah 1: Pilih Modul & Upload File</h4>
      
      <div class="form-group">
        <label>Pilih Modul:</label>
        <select id="importModule" onchange="onImportModuleChange()">
          <option value="">-- Pilih Modul --</option>
          <option value="sdmk">SDMK (Tenaga Kesehatan Terlatih)</option>
          <option value="pendaftaran">Pendaftaran Pelatihan</option>
          <option value="sertifikat">Sertifikat Pelatihan</option>
          <option value="pengumuman">Pengumuman</option>
          <option value="materi">Materi Pelatihan</option>
          <option value="indikator">Indikator Kinerja Program</option>
          <option value="admin">Admin/User</option>
        </select>
      </div>
      
      <div class="form-group">
        <label>Upload File Excel (.xlsx/.xls) atau CSV:</label>
        <div class="file-upload-zone" id="importDropZone">
          <i class="fas fa-cloud-upload-alt"></i>
          <p>Klik atau drag file ke sini</p>
          <p class="text-muted">Format: .xlsx, .xls, .csv (Maks 10MB)</p>
          <input type="file" id="importFileInput" accept=".xlsx,.xls,.csv" onchange="handleImportFileSelect(event)" hidden>
        </div>
        <button class="btn btn-primary" onclick="document.getElementById('importFileInput').click()">
          <i class="fas fa-folder-open"></i> Pilih File
        </button>
        <span id="selectedFileName" class="ml-2 text-muted"></span>
      </div>
      
      <div id="importTemplateInfo" class="info-box" style="display:none;">
        <i class="fas fa-info-circle"></i>
        <span id="templateInfoText"></span>
        <a href="#" onclick="downloadImportTemplate();return false;" class="btn btn-sm btn-secondary ml-2">
          <i class="fas fa-download"></i> Download Template
        </a>
      </div>
      
      <button class="btn btn-primary btn-block mt-3" id="btnPreviewImport" onclick="previewImportData()" disabled>
    ...
    <div id="importStep2" class="import-step" style="display:none;">
      <h4>Langkah 2: Preview Data & Konfirmasi</h4>
      
      <div class="preview-info">
        <span id="previewRowCount"></span> baris data ditemukan
        <span id="previewValidCount" class="text-success"></span>
        <span id="previewInvalidCount" class="text-danger"></span>
      </div>
      ...
      <div class="import-options mt-3">
        <label class="checkbox-inline">
          <input type="checkbox" id="importSkipHeader" checked> Baris pertama adalah header
        </label>
        <label class="checkbox-inline ml-3">
          <input type="checkbox" id="importSkipDuplicates"> Lewati data duplikat
        </label>
      </div>
      
      <div class="mt-3 d-flex gap-2">
        <button class="btn btn-secondary" onclick="backToImportStep1()">
          <i class="fas fa-arrow-left"></i> Kembali
        </button>
        <button class="btn btn-success" id="btnExecuteImport" onclick="executeBulkImport()">
          <i class="fas fa-database"></i> Import Sekarang (<span id="importCountDisplay">0</span> data)
        </button>
      </div>
    </div>
    ...
    <div id="importStep3" class="import-step" style="display:none;">
      <h4>Langkah 3: Hasil Import</h4>
      
      <div id="importResultSummary" class="result-summary"></div>
      
      <div id="importErrorLog" class="error-log" style="display:none;">
        <h5><i class="fas fa-exclamation-triangle"></i> Error Log:</h5>
        <pre id="importErrorDetails"></pre>
      </div>
      
      <button class="btn btn-primary btn-block mt-3" onclick="closeBulkImportModal();loadCurrentModuleData();">
        <i class="fas fa-check"></i> Selesai
      </button>
    </div>
    
    <!-- Progress Overlay -->
    <div id="importProgressOverlay" class="progress-overlay" style="display:none;">
      <div class="progress-content">
        <i class="fas fa-spinner fa-spin fa-3x"></i>
        <h4>Memproses Import...</h4>
        <p id="importStatusText">Mempersiapkan data...</p>
        <div class="progress-bar-container">
          <div class="progress-bar" id="importProgressBar" style="width:0%"></div>
        </div>
        <p id="importProgressText">0 / 0</p>
      </div>
    </div>
  </div>
</div>
...
<style>
/* ========== BULK IMPORT STYLES ========== */
/* FIX: Hide bulk import modal by default - only show when .active class added */
#bulkImportModal {
...
#bulkImportModal .modal-overlay {
  display: none;
}
#bulkImportModal.active .modal-overlay {
  display: flex;
}
...
.file-upload-zone i { font-size: 48px; color: var(--primary); margin-bottom: 15px; }
...
.result-summary.success { background: #fef2f2; border-color: #fca5a5; }
.result-summary i { font-size: 48px; margin-bottom: 10px; }
...
.d-flex { display: flex; }
.gap-2 { gap: 10px; }
.checkbox-inline { display: inline-flex; align-items: center; gap: 5px; cursor: pointer; }
